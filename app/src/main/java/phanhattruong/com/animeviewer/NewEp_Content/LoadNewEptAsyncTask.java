...
public class LoadNewEptAsyncTask extends AsyncTask<Void, String, Void> {
    ...
    protected Void doInBackground(Void... voids) {
        Document doc = null;
        try {
            doc = Jsoup.connect("https://vuighe.net/tap-moi-nhat").get();
            Element element = doc.body();
            Node AniNode = element.childNodes().get(3).childNodes().get(3);
    ...
    private ArrayList<AnimeInfo> GetAniInfoInList(Node AnilistNode) {
        ...
        for (int i = 0; i < AnilistNode.childNodeSize() / 2; i++) {
            ...
            String Name = NameChapViews.childNode(1).childNode(0).toString().replace("\n","");
            String Chap = NameChapViews.childNode(3).childNode(1).childNode(0).toString();
            String Views = NameChapViews.childNode(3).childNode(3).childNode(0).toString();
            String Source = AniInfo.attributes().get("href");
            AnimeInfo animeInfo = new AnimeInfo(Name,Chap,Source,ImageSource,Views,"null");
